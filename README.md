...
### 1️⃣ 관리자 회원가입
- 이름, 이메일, 비밀번호, 전화번호, 역할(슈퍼/운영/CS) 입력
- 입력값 유효성 검증 (이메일 형식, 비밀번호 8자 이상, 전화번호 `010-XXXX-XXXX`)
- 이메일 중복 검증 (중복 시 409)
- 비밀번호 BCrypt 암호화 저장
- 가입 시 **승인대기** 상태 부여 → 슈퍼 관리자 승인 후 로그인 가능

### 2️⃣ 관리자 인증 (세션/쿠키)
- 이메일 + 비밀번호 로그인, 인증 성공 시 서버 세션에 사용자 정보 저장
- 세션 ID를 **HttpOnly 쿠키**로 전달 (XSS 방지)
- 세션 유효 시간 24시간, 세션에 ID·이메일·역할 저장
- **활성** 상태만 로그인 가능 — 승인대기/거부/정지/비활성은 상태별 명확한 에러 반환
- 로그아웃 시 세션 무효화

### 3️⃣ 관리자 정보 관리
- 관리자 리스트 조회 — 검색(이름/이메일), 페이징, 정렬, 역할/상태 필터
- 관리자 상세 조회 / 정보 수정 / 역할 변경 / 상태 변경 / 삭제
- **가입 승인/거부** — 승인 시 승인일시 기록, 거부 시 거부 사유(필수)·거부일시 기록
- 내 프로필 조회/수정, 비밀번호 변경

### 4️⃣ 고객 정보 관리
- 고객 리스트 조회 — 검색, 페이징, 정렬, 상태 필터
- 고객 상세 조회 / 정보 수정 / 상태 변경 / 삭제

### 5️⃣ 상품 정보 관리
- 상품 등록 (등록 관리자 자동 저장) / 리스트 조회 / 상세 조회 / 수정 / 삭제
- **재고 자동 전환 정책**
  - 재고 0 이하 → `품절` 자동 전환
  - 재고 1 이상 → `판매중` 자동 전환
...
- 주문 생성 시 재고 검증·차감 및 상품 상태 자동 전환 (단일 트랜잭션)
- 주문 리스트 조회 — 검색(주문번호/고객명), 페이징, 정렬, 상태 필터
- 주문 상태 관리 — `준비중 → 배송중 → 배송완료` 순서 강제
- **주문 취소** — 준비중 상태만 취소 가능, 취소 사유 필수, 재고 자동 복구
...
- 1개의 주문 = 1개의 상품 종류 (학습 목적으로 간소화된 구조)
...
| 인증 | POST | `/admins/login`, `/admins/logout` |
| 관리자 | GET·PATCH·DELETE | `/admins`, `/admins/{id}`, `/admins/{id}/role·status·approve·reject`, `/admins/me`, `/admins/me/password` |
| 고객 | GET·PATCH·DELETE | `/customers`, `/customers/{id}`, `/customers/{id}/status` |
| 상품 | POST·GET·PATCH·DELETE | `/products`, `/products/{id}`, `/products/{id}/stock·status` |
| 주문 | POST·GET·PATCH | `/orders`, `/orders/{id}`, `/orders/{id}/status·cancel` |
...
도메인형 패키지 구조로, 도메인별 담당자가 독립적으로 작업할 수 있도록 설계했습니다.

## 🚀 실행 방법

```bash
# 1. 클론
git clone https://github.com/tls123qwe/CommercePJT.git
cd CommercePJT
...
```
main                      # 통합 브랜치
├── 1.feature/admin-signup
├── 2.feature/admin-auth
├── 3.feature/admin
├── 4.feature/customer
├── 5.feature/product
└── 6.feature/order
...
- **로그인 실패 시 이메일/비밀번호 구분 없는 에러 메시지** — 계정 존재 여부 노출 방지
